...
Exercises the complete BrainFlow API surface for the EDX board (id 67)
in a single 12-step sequence: connect, EEG stream, trigger output,
marker verification, impedance mode, mode switch recovery, stop,
release, and reconnect probe.
...
Usage:
    ...
    python python_package/examples/tests/test_edx_full_lifecycle.py --master-board 25 --verbose
"""
...
def parse_args():
    ...
    p.add_argument("--board-id", type=int, default=67, help="BrainFlow board id (default: 67)")
...
class LifecycleTest:
    def __init__(self, args):
        ...
        self.board = None
        self.data_board = args.master_board
        self.passed = []
        self.failed = []
    ...
    def test_connect(self):
        ...
        # Query capabilities to confirm connection
    ...
    def test_eeg_data(self):
        # Drain any startup data, then collect fresh
        ...
        # Check that EEG data is not all zeros
    ...
    def test_trigger_send(self):
        # Configure trigger: channel 0, 50% duty, 10Hz, 5 pulses, 1Hz burst, 3 bursts
    ...
    def test_trigger_receive(self):
        ...
        # Collect data over several seconds to catch trigger markers
        ...
        # With loopback, we'd expect markers. Without loopback, we still pass
        # but warn. The test validates the API doesn't crash.
        if total_markers == 0:
    ...
    def test_impedance_on(self):
        ...
        time.sleep(2.0)  # allow mode transition
    ...
    def test_impedance_data(self):
        ...
        # Check resistance channels from board descriptor
        try:
            ...
            print(f"  [WARN] Could not read resistance channels from descriptor: {e}")
            # Fall back to checking raw data is non-zero
            nonzero = np.count_nonzero(data) / data.size
            print(f"  Non-zero data fraction: {nonzero:.3f}")

    def test_impedance_off(self):
        resp = self.board.config_board("impedance_mode:0")
        print(f"  Response: {resp}")
        time.sleep(2.0)  # allow mode transition back to EEG
        print("  Waited 2s for EEG mode transition")

    def test_eeg_resumes(self):
        # Drain transition data
        self.board.get_board_data()
        time.sleep(1.0)
        data = self.board.get_board_data()
        n_samples = data.shape[1]
    ...
    def test_reconnect_probe(self):
        """Verify the amplifier was properly released by reconnecting."""
        ...
        print(f"  Probe session released cleanly")
